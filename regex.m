...
///    *
///  * `options` - an optional integer or table of integers and strings corresponding to values in the [hs.text.regex.matchOptions](#matchOptions) constant.
///    * if `options` is an integer, it should a combination of 1 or more of the numeric values in the [hs.text.regex.matchOptions](#matchOptions) constant logically OR'ed together (e.g. `hs.text.regex.matchOptions.reportProgress | hs.text.regex.matchOptions.withTransparentBounds`)
///    * if `options` is a table, each element of the array table should be a number value from the [hs.text.regex.matchOptions](#matchOptions) constant or a string matching one of the constant's keys. This method will logically OR the appropriate values together for you (e.g. `{"reportProgress", "withTransparentBounds"}`)
...
///  * `bg`       - an optional boolean, default `false`, specifying whether or not the expression matching should occur on a background thread (true) or not (false).
///
/// Returns:
///  * the regexObject
///
...
int luaopen_hs_text_regex(lua_State* L) {
    ...
    regex_regularExpressionOptions(L) ; lua_setfield(L, -2, "options") ;
